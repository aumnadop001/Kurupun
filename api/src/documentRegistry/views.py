...
def export_inventory_excel(request, document_registry_id):
    ...
    try:
        ...
        headers_row1 = [
            ...
            ('C1', 'หมายเลขพัสดุ', 1, 2),
            ('E1', '', 1, 4),
            ('I1', f'{doc_registry.registry_number} - {doc_registry.document_title}', 2, 6),
            ('O1', 'หน่วยนับ', 2, 2),
            ('Q1', 'ที่เก็บ', 2, 2),
        ]
        ...
        for record in inventory_records:
            ...
            ws[f'A{row}'] = record.doc_date_left.strftime('%d/%m/%Y') if record.doc_date_left else ''
            ...
            ws[f'I{row}'] = record.doc_date_right.strftime('%d/%m/%Y') if record.doc_date_right else ''
            ws[f'J{row}'] = record.received_qty if record.received_qty else ''
            ws[f'K{row}'] = float(record.price_per_unit) if record.price_per_unit else ''
